...
describe('getEmbeddingExecutionProfile', () => {
  ...
  test('hosted providers retain the existing cloud throughput defaults', async () => {
    await withEnv({ PMBRAIN_EMBED_CONCURRENCY: undefined, GBRAIN_EMBED_CONCURRENCY: undefined }, () => {
  ...
  test('Ollama starts with a bounded local profile and honors a lower operator cap', async () => {
    await withEnv({ PMBRAIN_EMBED_CONCURRENCY: undefined, GBRAIN_EMBED_CONCURRENCY: undefined }, async () => {
      ...
      await withEnv({ GBRAIN_EMBED_CONCURRENCY: '1' }, () => {
  ...
  test('timeout reduces Ollama concurrency and batch size, sustained success heals them', async () => {
    await withEnv({ PMBRAIN_EMBED_CONCURRENCY: undefined, GBRAIN_EMBED_CONCURRENCY: undefined }, () => {

...
## 2026-09-23 CI 分片串扰与桌面入口行数门禁误报

- 时间：2026-09-23
- 版本号：Core 1.3.86；Desktop 1.2.17
- 标题：修复 CI 测试隔离、桌面结构门禁和发布说明校验
- 描述：GitHub Test 的 shard 7 中，`embed-preflight` 的 `sk-test` 配置与 Dream Source 用例共享进程级 Gateway 状态，导致 Source 测试误发 OpenAI Embedding 请求并返回 401；现将该 Source 用例移入串行测试文件，并显式配置无凭证 Gateway。shard 3 中 Desktop 主入口增至 501 行，触发 500 行门槛；该门槛于 2026-08-09 随入口拆分建立为 450 行，2026-09-08 调至 500 行，目的是维持入口作为编排层的职责，但精确行数没有独立架构依据；现移除行数门槛，保留控制器委托和 IPC 边界断言。PR 预检还发现 Desktop 发布说明未覆盖当前版本且不满足最近五版校验，现同步补齐 1.2.16、1.2.17 并清理过旧条目。
- 是否完成：本地修复与门禁验证完成；新版本 GitHub exact-SHA CI 待用户提交后验证。
- 最终结果：Source 隔离用例 4/4、结构测试 4/4、Desktop 测试 299/299、`verify` 39/39、本地 PR 预检 233/233 通过；版本同步检查通过。历史失败 SHA `8474b497d0c97ef96090386ae40413821c20cdfa` 的 Test 日志已确认上述两个失败；修复尚未推送，因此新 SHA 的 GitHub CI 结果待验证。未修改知识库、Wiki、原始资料、向量或 Schema，未执行 `build:win`。

## 2026-09-22 普通文件夹 Source 被错误要求初始化 Git

- 时间：2026-09-22
- 版本号：Core 1.3.84；Desktop 1.2.15
- 标题：修复非 Git 数据源同步失败及 CI runner 偶发失败
- 描述：本地路径 Source 没有配置远程仓库时仍被统一送入 Git HEAD、工作区和提交差异检查，导致 `D:\tmp` 这类普通文件夹在快速维护中直接报 `Not a git repository`。现按 Source 配置分流：带 `remote_url` 的来源继续保留 Git 校验与增量同步；普通文件夹按当前目录快照完整扫描，复用既有导入、失败门禁、Source 隔离和可恢复软删除，不创建 Git 仓库、不执行 Git HEAD、工作区或 `.gitignore` 操作。同步补齐普通文件夹完整扫描对图片包含选项的传递。首个新提交的 GitHub Test 暴露出 Google 分流测试依赖旧 Bug 提前退出，修复后会继续进入导入并在无向量模型的 CI 环境退出；现为该测试显式指定 `noEmbed`，只验证其原本负责的“本地路径不走 Google 凭证”合同。
- 是否完成：是，本地修复、定向回归、类型检查、核心门禁、PR 预检和 GitHub exact-SHA CI 均已完成。
- 最终结果：普通文件夹首次同步、重复扫描和文件删除回归 1/1 通过；普通文件夹、Git 与 Google 分流定向测试 5/5，相关 Git/多 Source/并行同步测试合计 19/19 通过，Postgres 条件用例因本机未配置测试数据库跳过 34 项；Core 类型检查、`verify` 39/39、本地 PR 预检 233/233 通过。提交 `8e9919e4baa2938c9117848866c441bd41033075` 的 GitHub Test、Core User Journeys、E2E Tests、Heavy Tests 全部通过。未修改用户知识库、Wiki、原始资料、向量或 Schema，未执行 `build:win`。

## 2026-09-22 工作台选项回退与 CI 产品合同不一致

- 时间：2026-09-22
- 版本号：Core 1.3.83；Desktop 1.2.15
- 标题：修复写入位置被重新显示及 Test 分片失败
- 描述：1.3.82 将已撤下的写入位置重新加入知识工作台，既违背已确认的全自动交互，也直接触发 GitHub Test 中“工作台不再展示写入与处理选项”的产品合同失败。现删除该选择器及显式 Source 参数，恢复由主知识源统一承接导入，并同步清理样式和回归测试。Core User Journeys 复跑后确认首次设置先按推荐维度建库、再探测到自定义模型真实维度，导致导入以维度不匹配失败；旧旅程又把查询摘要中的关键词误判为检索命中。现首次激活统一以 `--empty-only` 安全对齐空向量库，并要求旅程验证真实结果列表。发布说明同时恢复只保留最近五版的 CI 合同。
- 是否完成：本地修复、完整用户路径与本地 CI 门禁已完成；GitHub 新提交 exact-SHA CI 待用户提交后验证。
- 最终结果：Desktop 启动策略 22/22、工作台/模型/健康页定向测试 56/56、核心用户路径 6/6、发布门禁 6/6、`verify` 39/39、本地 PR 预检 233/233 通过。已有向量时 `--empty-only` 仍会拒绝自动清理；未修改用户数据，未执行 `build:win`。

## 2026-09-22 年表补录假成功、健康检查无反馈与 CI 回归

- 时间：2026-09-22
- 版本号：Core 1.3.82；Desktop 1.2.14
- 标题：修复年表任务未执行、知识列表竞态及当前 CI 旧合同
- 描述：年表补录原先只创建后台任务却没有启动 Worker，界面又在入队后立即宣称成功，导致刷新后同一建议一直存在；现入队后启动既有 Worker，界面持续检查真实健康结果，只有缺口消失才提示完成，超时或失败则明确引导查看任务中心。“重新检查”增加检查中、禁用和完成反馈。知识库列表增加请求序号，避免旧请求晚返回后覆盖新筛选结果。同步修复当前 Test/Core User Journeys 暴露的能力过滤、Schema 数量、版本、测试状态污染、Desktop 入口体积和页面列表竞态合同。
- 是否完成：本地修复与回归验证完成；GitHub 新提交 exact-SHA CI 待用户提交后验证。
- 最终结果：补入年表不再假成功，重新检查有可见反馈，最新列表请求不会被旧响应覆盖；无变化 Source 同步沿用 1.3.81 修复，扫描成功即更新时间并清除过期建议。项目 `verify` 39/39、本地 PR 预检 233/233、Core/Desktop 类型检查、Admin/Desktop 源码构建和最新源码 Playwright 交互验证通过。未执行历史批量回填、用户数据修改或 `build:win`。

## 2026-09-22 Source 同步时间、图片 Source 隔离与 Windows 首次配置卡死

- 时间：2026-09-22
- 版本号：Core 1.3.81；Desktop 1.2.13
- 标题：修复同步完成仍显示过期、图片向量化找不到页面及核心用户旅程超时
- 描述：Source 没有文件变化时原同步流程会提前返回，却不刷新 `last_sync_at`，所以任务几秒完成后首页仍长期显示“未同步”；现成功完成扫描即刷新 Source 最近同步时间。独立图片导入原先按目标 Source 查找页面，但事务仍把页面、切片和文件写进默认 Source，随后在目标 Source 向量化时报 `Page not found`；现整条图片事务及兄弟页面链接都显式传递同一 Source。GitHub Core User Journeys run `35602262365` 的 Windows 真实界面日志确认首次配置停在空向量库对齐；全新 PGLite 已按刚保存的模型维度创建，却又执行重复对齐并偶发卡死，现只对既有 PGLite 保留 empty-only 对齐，全新库直接继续启动，Postgres 流程不变。
- 是否完成：本地修复与回归验证完成；GitHub 新提交 exact-SHA CI 待用户提交后验证。
- 最终结果：无变化同步会更新健康检查所依赖的同步时间；图片页面、切片和文件均写入请求的 Source，OCR 不支持时仍只跳过识别而不阻断导入；全新 PGLite 不再重复对齐，既有库的数据安全边界不变。相关 Admin/Core 定向测试 33/33、图片真实 PGLite 测试 8/8、Desktop 全量测试 294/294、`verify` 39/39、源码构建、版本同步和本地 PR 预检 233/233 通过。未删除或回填用户数据，未执行 `build:win`。

## 2026-09-21 图片识别配置重复、导入未识别、CI 与 AI SDK Warning

- 时间：2026-09-21
- 版本号：Core 1.3.80；Desktop 1.2.12
- 标题：修复图片内容未进入知识链及视觉请求 system message 兼容问题
- 描述：原独立图片导入依赖多模态向量模型，PDF/Office 图片识别只在局部手工入口生效，Source 与快速维护也无法依据 OCR 状态重试；设置页又重复要求用户选择“复用普通模型”并单独启用，未体现图片模型为空时应自然继承普通模型。视觉请求还把 system message 放在 messages 数组中，触发新版 AI SDK Warning。现把 OCR 下沉为 Gateway 和文档导入公共能力，删除重复开关与来源选择，system 改用顶层参数，增加已知模型视觉能力检测、真实图片连接测试、识别回执和维护过期判断；AI 深度整理与快速维护均默认包含 Office 并自动尝试图片识别，知识工作台移除所有文件处理勾选项，普通模型调用开关一并移除。OCR 不可用或单页失败时保留正常文字导入并明确提示。同时修复 CI 中 5 项测试隔离违规、operations 重复类型导出、链接数组隐式 any、LongMemEval 元数据、autocut 回放配置和 Open Loops 空值断言。
- 是否完成：本地修复、回归测试和源码构建已完成；真实云端视觉模型与发布后安装包待验证。
- 最终结果：OCR 路由、图片输入、PDF/Office、Source 同步、快速维护、配置持久化和导入摘要等定向测试 166/166 通过，独立图片详情 7/7、自动化默认行为新增契约 33/33、Desktop 全量测试 291/291、CI 修复定向 56/56 通过；根与 Desktop TypeScript、测试隔离、Admin/Desktop 源码构建、版本同步、最新源码页面实测和全项目 `verify` 39/39 通过。本次未改 Schema、用户数据或向量，未执行 `build:win`；真实外部视觉模型、Postgres 与 GitHub exact-SHA CI 待后续验证。

## 2026-09-20 连接器整行 Cookie 无法识别与报错不清

- 时间：2026-09-20
- 版本号：Core 1.3.74；Desktop 1.2.6
- 标题：修复 ChatGPT、Claude 粘贴整行 Cookie 后验证失败，并区分真实失败原因
- 描述：界面原先要求复制 Cookie 的值，服务端却会把用户粘贴的整行 `Cookie:` 原样当成请求头值，造成无效格式；凭证失效、Cloudflare 拦截、网络失败和上游接口变化又全部显示为同一条错误。现统一指引用户从 Network 的 Request Headers 复制整行 `Cookie:`，Admin、Desktop 和 CLI 均会去掉头名；误粘贴多行时仅提取 Cookie，不发送其他请求头。界面改为分类告知失败原因。
- 是否完成：本地代码、构建和隔离管理台页面验收已完成；用户真实会话及 GitHub CI 待验证。
- 最终结果：连接器定向测试 22/22、Desktop 日常页与 Google IPC 测试 32/32、Admin 生产构建、Desktop 源码构建及版本同步通过；最新源码隔离页面已确认完整展示 Network → Request Headers 复制整行 `Cookie:` 的指引。`verify` 37/39，仅余与本次无关的已有测试隔离违规和 TypeScript 错误。旧版将探测结果丢失后只留下通用错误，因此不能从旧截图反推当次究竟是凭证失效还是 Cloudflare、网络或接口变化；新版重试会直接显示具体类型。未使用用户 Cookie 发起真实请求，未执行 `build:win`。

## 2026-09-19 Docker 迁移兼容旧库、修复容器发现切换与 CI 误报

- 时间：2026-09-18 至 2026-09-19
- 版本号：Core 1.3.70；Desktop 1.2.2
- 标题：修复完整知识库迁移、Docker 停止数据库无法发现切换及 CI 旧断言
- 描述：原迁移对 `content_chunks_keep` 历史备份表一律要求目标库有对应表，导致真实旧库在 119 项 Schema 初始化完成后阻断；旧库个别 Facts 记录有重复 ID，也会触发目标主键冲突。现由集中兼容注册表识别历史备份并明确报告跳过数量；无引用歧义的重复 Facts 在新库保留全部内容并分配新 ID，预检先告知，无法安全判断时继续阻断。新建容器改为容器内 TCP `pg_isready` 与真实 `SELECT 1` 同轮成功后才继续。迁移后回显当前地址。数据库发现不再把停止状态直接过滤：管理标签和 `gbrain-pg`、`pmbrain-postgres-*` 历史名称可进入列表，选择时启动并完整验证，失败恢复原停止状态。CI 同时移除依赖单行源码格式的迁移测试，并让预算测试显式指定计价模型，避免读取开发机本地模型配置后误报。
- 是否完成：本地代码、隔离数据库、真实 Docker 发现、界面预览及本地门禁验证完成；正式安装版与 GitHub CI 待用户提交、打包后验证。
- 最终结果：真实知识库隔离副本的 59 张正式表、41,518 行迁移并校验成功；原 PGLite、历史备份及用户现有 Docker 容器未修改。本机真实停止容器 `gbrain-pg` 已被发现，选择后自动启动、通过核心表验证，验收结束后恢复原停止状态；无关停止容器被排除，失败验证会自动停止刚启动的候选。数据库运行时与界面测试 46/46、相关 CI 测试 11/11、Desktop 类型检查和源码构建、深色界面预览、`verify` 39/39、`ci:pr-preview` 232/232 通过。未执行正式数据库切换、`build:win`、提交或推送。
...
- 时间：2026-09-15
- 版本号：Core 1.3.64；Desktop 1.1.95
- 标题：会话目录可直接导入且不限制 JSONL 原始大小，修复同步超时与健康建议缺少入口
- 描述：目录导入原先只扫描资料文件，单文件导入虽支持 Codex、Grok JSONL，粘贴 sessions 目录却会得到 0 个文件；会话解析同时受 20 MB 限制，Admin 原始请求又受 50 MB 内存上传限制。现仅在会话导入路径识别 JSONL，改用临时文件流式上传和逐行解析，不再限制会话原始文件大小，普通资料仍保留 50 MB 保护。Source 同步复用既有命令和任务中心，取消固定 10 分钟终止；健康建议中的缺失链接改为中文，并在知识图谱提供只读明细。另修复首次隔离配置提前启动 Sidecar 造成 PGLite 自占用，以及知识分类测试不符合 CI 隔离合同的问题。
- 是否完成：是，代码与本地验证完成；新版 GitHub CI 和 Windows 安装包待用户提交后验收。
- 最终结果：定向测试 91/91、Core 与 Desktop 类型检查、Admin 与 Desktop 源码构建、`verify` 39/39、`ci:pr-preview` 232/232、Desktop 核心用户旅程 6/6 通过。真实 Codex sessions 目录识别 6 个 JSONL，141.1 MB 日志经界面成功上传并流式解析出 127 条用户/助手消息。缺失链接只读查看不自动删除或回填关系；未修改用户知识库、Wiki、原始资料、向量或数据库，未提交 GitHub，未执行 `build:win`。

## 2026-09-14 知识栏目来源分类、CI 回归与查询通道恢复

- 时间：2026-09-14
- 版本号：Core 1.3.63；Desktop 1.1.94
- 标题：原始资料与加工知识分开展示，修复已定位的 CI 失败
- 描述：栏目改按已核实的 dream_generated、extracted_by、synthesized_by 产物标记判定，保留 type、观点与事实语义；无可靠标记的旧内容归原始资料，抽取回执不算结构化知识。SQL 在分页前过滤并保留 Source 与稳定时间排序。修复 Test run 34743734505 的发布说明版本缺失及 init 测试旧 Home 路径；修复 Core User Journeys run 34743734512 仍要求 FIRST RUN 的旧断言，改为检查首次自动创建的隔离配置和数据库。
- 是否完成：代码与本地定向验证完成；新版 GitHub CI、Windows 安装包旅程待提交后验收。
- 最终结果：分类/发布/管理台/旅程合同 17 项通过；init 等定向组合 14 项通过；分类在 PGLite 与独立 Postgres 各 13 条断言通过，既有事实排序 Postgres 对等测试通过。Core/Desktop 类型检查、Python 语法检查、版本同步、Admin/Desktop/Sidecar 构建通过；最新源码隔离页面实测分类正确。查询通道检查发现本地 MCP 正常、既有 tunnel-client 未运行，重启原隧道后同一 PMBrain search 返回真实结果；历史 429 缺少当次日志，不能归因为额度耗尽。本轮未改隧道权限或自动启动策略，未修改用户知识库数据，未提交 GitHub，未运行 build:win。

## 2026-09-13 自定义本地模型冷启动连接测试过早超时

- 时间：2026-09-13
- 版本号：Desktop 1.1.93；Core 1.3.62
- 标题：自定义端点沿用本地模型冷启动等待时间
- 描述：自定义 OpenAI 兼容端点及保存的 custom-endpoint 标识，连接测试由 15 秒改为 120 秒；普通与向量模型共同适用，界面提示首次加载需要 1–2 分钟。复用现有 Gateway、端点与凭据隔离逻辑，不增加预加载协议、回退模型或修改远程服务配置。
- 是否完成：本地修复完成，真实远程主机与发布验收待确认。
- 最终结果：模型连接、端点配置、模型目录、老用户路由及高级配置 32 项通过；包含模拟服务延迟 15.2 秒后成功返回。Core/Desktop 类型检查、Desktop/Admin/Sidecar 资源构建通过。未在反馈中的远程 RTX4090 主机实测；服务本身仍需启动，模型必须已下载。未提交 GitHub，未执行 build:win，未修改用户知识库数据。

## 2026-09-13 知识数据与任务列表时间排序修复

- 时间：2026-09-13
- 版本号：Core 1.3.62；Desktop 1.1.93
- 标题：事实按显示的更新时间倒序，时间相同的记录保持稳定顺序
- 描述：事实原来显示 embedded_at/created_at 合成的更新时间，却只按 created_at 排序，现显示与排序采用同一表达式；全部、原始与资料、结构化知识、观点与总结及回收站用唯一 ID 打破时间并列；任务按完成时间或开始时间倒序；知识整理的后台任务及评估记录补齐唯一键排序。查询在分页前排序，不修改数据或 Schema。
- 是否完成：本地修复完成；Postgres 与 GitHub CI 待验证。
- 最终结果：相关 Core 定向 55 项通过，包含 PGLite 事实 10 项；组合测试曾在 PGLite 初始化超时，原断言单独重跑通过。最新源码隔离管理台已打开，确认旧事实新更新后位于较新创建记录之前，验证后关闭临时服务。请求日志、观点提案已有时间倒序；搜索相关性、图谱、统计排行和详情内部顺序不改。Postgres 对等测试已加入，本机 Docker 服务未运行，未实测。Core/Desktop 类型检查、版本同步、Admin/Desktop/Sidecar 构建和打包运行时检查通过，未提交或打 Windows 安装包，未修改用户真实数据。

## 2026-09-10 PMBrain 1.3.60 MCP 接入状态每次打开都停在待验证

- 时间：2026-09-10
- 版本号：Core 1.3.60；Desktop 1.1.91
- 标题：接入页在服务就绪后复核并保留真实验证结果
- 描述：桌面页可能在 Sidecar 就绪前完成一次没有连接状态的探测，却仍记为验证完成；随后基础配置接口返回的旧快照还能覆盖已完成的连接验证。CodeBuddy、Cursor、Trae 又只检查配置文件存在，从不校验 Bearer，因此每次打开都显示“已写入，待验证”。现保存不含凭证的最近验证回执，页面打开时优先沿用“接入可用”并在后台静默复核；Sidecar 真正就绪后自动再查，旧快照不覆盖新结果，所有自动 JSON 客户端统一复用限时 Bearer 校验。明确 401/403 才显示“接入失效”，临时超时或服务不可用显示“待验证”。
- 是否完成：是，代码与本地验证完成；GitHub CI 待本次提交运行。
- 最终结果：本机只读实测已得到明确状态：WorkBuddy、Cursor、Trae、Codex 接入可用；CodeBuddy、Claude、Grok 现存凭证失效；QwenPaw 已写入但尚未连通。Desktop 263/263、定向门禁 14/14、Core/Desktop TypeScript、桌面源码构建、版本同步、`verify` 39/39、`ci:pr-preview` 232/232 通过。未修改上述客户端配置和凭证，未执行 `build:win`，未修改知识库、Wiki、原始资料、向量或数据库。

## 2026-09-10 PMBrain 1.3.59 Ubuntu CI 跨平台路径与旧 Home 兼容断言错误

- 时间：2026-09-10
- 版本号：Core 1.3.59；Desktop 1.1.90
- 标题：修复 Test 工作流分片 8、9 的跨平台回归误报
- 描述：精确检查 GitHub Test run `34432128205` 后确认两个失败均为测试合同问题：新增 Sidecar 回归测试写死 Windows 反斜杠路径，Ubuntu 无法识别文件名；Home 隔离测试虽声明兼容既有 `~/.gbrain`，却仍无条件断言 `~/.pmbrain`。现改用平台原生路径构造，并按生产实现的迁移优先级验证 PMBrain Home 与既有 GBrain Home；未改变生产逻辑。
- 是否完成：是，代码与本地验证完成；GitHub CI 待本次提交运行。
- 最终结果：跨平台与 Home 兼容定向门禁包含在 14/14 通过项中；Core/Desktop TypeScript、桌面源码构建、版本同步、`verify` 39/39、`ci:pr-preview` 232/232 通过。未执行 `build:win`，未修改知识库、Wiki、原始资料、向量或数据库。
...
- 时间：2026-09-10
- 版本号：Core 1.3.57；Desktop 1.1.88
- 标题：接入操作改为即时分阶段弹窗，直接验证不再等待全客户端轮询
- 描述：点击“更新”后，桌面端把创建凭证、MCP smoke、配置写入、深度规则安装和全客户端连接探测都压成“正在验证”，且配置结果显示在页面底部；直接写入成功的绿色横幅还可能与随后返回的失效卡片并存。现把按钮区分为“接入 / 更新连接 / 重新生成凭证”，点击后立即弹出当前客户端专属进度；普通更新明确只更新 MCP，深度接入明确同时安装自动记忆规则，首次长期记忆选择只询问一次，后续统一在系统设置修改。刚写入且 smoke 通过的自动配置立即返回“凭证可用”，全客户端复核改为后台执行，不再阻塞当前操作结果。
- 是否完成：是
- 最终结果：Desktop 测试 260/260、TypeScript、桌面构建、版本同步和源码暗色界面预览通过；`verify` 39/39、`ci:pr-preview` 232/232 通过。预览核对了 MCP 卡片与 Codex 深度接入完成弹窗。未执行 `build:win`，未修改知识库、Wiki、原始资料、向量或已有 Facts；GitHub Ubuntu/Postgres E2E/Heavy/NSIS 待用户提交后验证。
...
- 时间：2026-09-04
- 版本号：PMBrain 1.3.41；PMBrain Desktop 1.1.73
- 标题：1.3.40 只升了后端版本，管理台生产资源未重建
- 描述：用户更新到 v1.3.40 后，总体概览仍是「可用于 AI 搜索」和「待处理 ↑」，没有「已向量化」和 +/-。原因是源码改了，但打包用的 `admin/dist` 和内嵌资源还是旧的。现重建管理台生产资源，并加契约防止以后源码改了包装还是旧界面。未修改用户知识、向量、Wiki 和原始资料。
- 是否完成：是
- 最终结果：已重建 Admin 生产资源并写入内嵌包，指纹 `eb6088e643c2`。定向测试含「安装包必须带上已向量化」契约，18/18 通过。未执行 `bun run build:win`。当前 v1.3.40 安装包仍是旧界面，需再打包 1.3.41 后才会变。
...
- 时间：2026-08-23
- 版本号：PMBrain 1.2.90；PMBrain Desktop 1.1.44
...
- 时间：2026-08-22
- 版本号：PMBrain 1.2.87；PMBrain Desktop 1.1.43（桌面端未改动）
- 标题：后台向量补全打印成功后若子进程不退出则强制收尾
- 描述：GitHub 核心用户路径中，打包后的 `embed --stale --catch-up` 已写出 `{"status":"ok"}` 并完成 2 个分块，但子进程不退出。该任务关闭了外层超时，父进程一直等待 close，任务中心保持 running，Admin 持续 423 Locked。现对已打印终端 JSON 结果、超过 15 秒仍未退出的子进程强制结束；成功 JSON 记为已完成，失败 JSON 记为失败。进度事件 JSON 不触发收尾。未修改用户知识、向量、数据库、Wiki 或原始资料。
- 是否完成：是
- 最终结果：失败优先测试覆盖成功 JSON 挂起后强制完成、失败 JSON 挂起后记失败、进度事件不误杀；执行器钩子 9/9 通过。未执行 `bun run build:win`。GitHub Core User Journeys 待推送后验证。

...
## 2026-09-19 Docker 迁移兼容旧库并修复新容器就绪竞态

- 时间：2026-09-18 至 2026-09-19
- 版本号：Core 1.3.68；Desktop 1.2.0
- 标题：修复完整知识库迁移的历史 Schema、旧库主键及 Docker 新容器就绪竞态
- 描述：原迁移对 `content_chunks_keep` 历史备份表一律要求目标库有对应表，导致真实旧库在 119 项 Schema 初始化完成后阻断；旧库个别 Facts 记录有重复 ID，也会触发目标主键冲突。现由集中兼容注册表识别历史备份并明确报告跳过数量；无引用歧义的重复 Facts 在新库保留全部内容并分配新 ID，预检先告知，无法安全判断时继续阻断。新建容器原先用未指定 Host 的 `pg_isready`，可能命中官方镜像首次初始化的临时 Unix socket 服务；现改为容器内 TCP `pg_isready` 与真实 `SELECT 1` 同轮成功后才继续，并由 PMBrain Schema 统一创建扩展。Docker Desktop 引擎启动等待保持 180 秒。
- 是否完成：本地代码及隔离数据库验证完成；正式安装版与 GitHub CI 待用户提交、打包后验证。
- 最终结果：真实知识库隔离副本的 59 张正式表、41,518 行迁移并校验成功；原 PGLite、历史备份及用户现有 Docker 容器未修改。新增“临时服务可见、SQL 探测失败、正式服务可查询后才放行”的竞态测试，并用全新 Docker Volume 确认运行时不提前创建扩展、Schema 正确创建 `vector`、`pg_trgm`、`pgcrypto`，随后完成真实 Postgres 跨库迁移。Core 跨库定向测试 7/7、Desktop 相关测试 18/18、`verify` 39/39、`ci:pr-preview` 232/232、Desktop 类型检查与源码构建、Sidecar 组装和 Windows Runtime 检查通过。未执行正式用户库切换、`build:win` 或推送。

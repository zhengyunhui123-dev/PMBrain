...
## 2026-09-18 Docker 迁移遇历史备份表与旧库重复 Facts 而失败

- 时间：2026-09-18
- 版本号：Core 1.3.68；Desktop 1.1.99
- 标题：修复完整知识库迁移对历史 Schema 与旧库主键异常不兼容
- 描述：原迁移对 `content_chunks_keep` 历史备份表一律要求目标库有对应表，导致真实旧库在 119 项 Schema 初始化完成后阻断；旧库个别 Facts 记录有重复 ID，也会触发目标主键冲突。现由集中兼容注册表识别历史备份并明确报告跳过数量；无引用歧义的重复 Facts 在新库保留全部内容并分配新 ID，预检先告知，无法安全判断时继续阻断。Docker Desktop 自动启动后的就绪等待从 90 秒延长到 180 秒。
- 是否完成：本地代码及隔离数据库验证完成；正式安装版与 GitHub CI 待用户提交、打包后验证。
- 最终结果：真实知识库隔离副本的 59 张正式表、41,518 行迁移并校验成功；原 PGLite、历史备份及用户现有 Docker 容器未修改。Core 跨库定向测试 7/7、Desktop 相关测试 17/17、本地 `verify` 39/39 与 `ci:pr-preview` 232/232 通过。未执行正式用户库切换、`build:win` 或推送。

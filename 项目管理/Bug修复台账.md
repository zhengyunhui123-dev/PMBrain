...
- 描述：本地路径 Source 没有配置远程仓库时仍被统一送入 Git HEAD、工作区和提交差异检查，导致 `D:\tmp` 这类普通文件夹在快速维护中直接报 `Not a git repository`。现按 Source 配置分流：带 `remote_url` 的来源继续保留 Git 校验与增量同步；普通文件夹按当前目录快照完整扫描，复用既有导入、失败门禁、Source 隔离和可恢复软删除，不创建 Git 仓库、不执行 Git HEAD、工作区或 `.gitignore` 操作。同步补齐普通文件夹完整扫描对图片包含选项的传递。当前 GitHub Test 旧提交仅有一个托管 runner 在 `Set up job` 阶段失败，没有进入代码步骤，其余分片与工作流通过；不修改产品代码或测试合同规避基础设施失败，改由新提交 exact-SHA 重新验收。
- 是否完成：本地修复、定向回归、类型检查、核心门禁和 PR 预检已完成；GitHub 新提交 exact-SHA CI 待提交后验证。
- 最终结果：普通文件夹首次同步、重复扫描和文件删除回归 1/1 通过；相关 Git/多 Source/并行同步测试合计 19/19 通过，Postgres 条件用例因本机未配置测试数据库跳过 34 项；Core 类型检查、`verify` 39/39、本地 PR 预检 233/233 通过。未修改用户知识库、Wiki、原始资料、向量或 Schema，未执行 `build:win`。

...
- 时间：2026-09-21
- 版本号：Core 1.3.79；Desktop 1.2.11
- 标题：修复图片内容未进入知识链及视觉请求 system message 兼容问题
- 描述：原独立图片导入依赖多模态向量模型，PDF/Office 图片识别只在局部手工入口生效，Source 与快速维护也无法依据 OCR 状态重试；设置页又重复要求用户选择“复用普通模型”并单独启用，未体现图片模型为空时应自然继承普通模型。视觉请求还把 system message 放在 messages 数组中，触发新版 AI SDK Warning。现把 OCR 下沉为 Gateway 和文档导入公共能力，删除重复开关与来源选择，system 改用顶层参数，增加已知模型视觉能力检测、真实图片连接测试、识别回执和维护过期判断。OCR 不可用或单页失败时保留正常文字导入并明确提示。同时修复 CI 中 5 项测试隔离违规、operations 重复类型导出、链接数组隐式 any、LongMemEval 元数据、autocut 回放配置和 Open Loops 空值断言。
- 是否完成：本地修复、回归测试和源码构建已完成；真实云端视觉模型与发布后安装包待验证。
- 最终结果：OCR 路由、图片输入、PDF/Office、Source 同步、快速维护、配置持久化和导入摘要等定向测试 166/166 通过，独立图片详情 7/7、最终 Desktop 65/65、CI 修复定向 56/56 通过；根 TypeScript、测试隔离、版本同步和全项目 `verify` 39/39 通过。本次未改 Schema、用户数据或向量，未执行 `build:win`。

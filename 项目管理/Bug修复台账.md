...
## 2026-07-28 修复跨平台识别旧 Dream 完成记录失败

- 时间：2026-07-28
- 版本号：PMBrain 1.1.75
- 标题：让 Windows 语料路径在 Linux/macOS 上仍能匹配旧 Dream 任务
- 描述：语料目录移动后的幂等判断使用运行平台原生 `basename`；Windows 反斜杠路径进入 Linux/macOS runner 后会被当成完整文件名，导致同一文件的旧路径与新路径无法匹配，并使合并后的 CI 回归失败。
- 是否完成：是
- 最终结果：比较旧 Dream idempotency key 与当前文件路径前先统一路径分隔符，Windows、Linux 和 macOS 均按真实文件名匹配；已有单任务与完整分块任务识别规则不变，不修改知识库、原始资料或既有任务记录。

...
export class PostgresEngine implements BrainEngine {
  ...
  private async applyForwardReferenceBootstrap(injectedConn?: postgres.Sql): Promise<void> {
    ...
    const probeRows = await conn<{
      ...
      SELECT
        ...
        EXISTS (SELECT 1 FROM information_schema.columns
                WHERE table_schema = current_schema() AND table_name = 'minion_jobs' AND column_name = 'private_queue_owner_job_id') AS minion_jobs_private_queue_owner_job_id_exists,
        EXISTS (SELECT 1 FROM information_schema.columns
                WHERE table_schema = current_schema() AND table_name = 'minion_jobs' AND column_name = 'private_queue_owner_token') AS minion_jobs_private_queue_owner_token_exists,
        EXISTS (SELECT 1 FROM information_schema.columns
                WHERE table_schema = current_schema() AND table_name = 'minion_jobs' AND column_name = 'private_queue_lease_until') AS minion_jobs_private_queue_lease_until_exists
    `;
    ...
    const needsPagesLastRetrievedAt = probe.pages_exists && probe.pages_last_retrieved_at_exists !== true;
    // Always treat missing/undefined probe as "need the column". Existing
    // brains must get ADD COLUMN before SCHEMA_SQL creates the index.
    const needsPagesLinksExtractedAt = probe.pages_exists && probe.pages_links_extracted_at_exists !== true;
    ...
    const needsMinionJobsBootstrap = probeCr.minion_jobs_exists === true
      && (probeCr.minion_jobs_timeout_at_exists !== true
        || probeCr.minion_jobs_idempotency_key_exists !== true
        || probeCr.minion_jobs_private_queue_owner_job_id_exists !== true
        || probeCr.minion_jobs_private_queue_owner_token_exists !== true
        || probeCr.minion_jobs_private_queue_lease_until_exists !== true);
    ...
    if (needsPagesLinksExtractedAt) {
      // Schema 115: pages_links_extracted_at_idx in SCHEMA_SQL references
      // links_extracted_at. Existing brains that already have every older
      // bootstrap target still lack this column; omitting it from the
      // early-return above made Sidecar skip ADD COLUMN and crash on
      // schema replay.
      await conn.unsafe(`
  ...
  async searchKeyword(query: string, opts?: SearchOpts): Promise<SearchResult[]> {
    ...
    if (opts?.limit && opts.limit > MAX_SEARCH_LIMIT) {
      console.warn(`[pmbrain] Warning: search limit clamped from ${opts.limit} to ${MAX_SEARCH_LIMIT}`);
    }
  ...
  async countStalePagesForExtraction(opts?: { sourceId?: string; versionTs?: string }): Promise<number> {
    try {
      ...
      const rows = await this.executeRaw<{ count: number }>(
        `SELECT count(*)::int AS count FROM pages WHERE ${where}`,
        params,
      );
      return rows[0]?.count ?? 0;
    } catch (error) {
      if (isUndefinedColumnError(error, 'links_extracted_at')) return 0;
      throw error;
    }
  }
  ...
  async sweepDreamVerdicts(): Promise<number> {
    const result = await this.sql`DELETE FROM dream_verdicts WHERE expires_at <= now()`;
    ...
    const [stats] = await sql`
      SELECT
        (SELECT count(*) FROM pages p
          WHERE p.deleted_at IS NULL AND (${scope}::text[] IS NULL OR p.source_id = ANY(${scope}))) AS page_count,
        (SELECT count(*) FROM content_chunks c JOIN pages p ON p.id = c.page_id
          WHERE p.deleted_at IS NULL AND (${scope}::text[] IS NULL OR p.source_id = ANY(${scope}))) AS chunk_count,
        (SELECT count(*) FROM content_chunks c JOIN pages p ON p.id = c.page_id
        ...
        (SELECT count(*) FROM links l
          ...
          WHERE ${scope}::text[] IS NULL OR (pf.source_id = ANY(${scope}) AND pt.source_id = ANY(${scope}))) AS link_count,
        (SELECT count(DISTINCT t.tag) FROM tags t JOIN pages p ON p.id = t.page_id
          WHERE p.deleted_at IS NULL AND (${scope}::text[] IS NULL OR p.source_id = ANY(${scope}))) AS tag_count,
        (SELECT count(*) FROM timeline_entries te JOIN pages p ON p.id = te.page_id
          WHERE p.deleted_at IS NULL AND (${scope}::text[] IS NULL OR p.source_id = ANY(${scope}))) AS timeline_entry_count
    `;

    const types = await sql`
      SELECT type, count(*)::int AS count FROM pages p
      WHERE p.deleted_at IS NULL AND (${scope}::text[] IS NULL OR p.source_id = ANY(${scope}))
      GROUP BY type ORDER BY count DESC
    `;
    const pages_by_type: Record<string, number> = {};
    for (const row of types) pages_by_type[row.type as string] = Number(row.count);
    return {
  ...
  async getHealth(opts?: { sourceId?: string; sourceIds?: string[] }): Promise<BrainHealth> {
    ...
    const connected = await sql`
      SELECT p.slug,
        (SELECT count(*) FROM links l
          WHERE (l.from_page_id = p.id AND (${scope}::text[] IS NULL OR EXISTS (
                   SELECT 1 FROM pages far WHERE far.id = l.to_page_id AND far.deleted_at IS NULL AND far.source_id = ANY(${scope}))))
             OR (l.to_page_id = p.id AND (${scope}::text[] IS NULL OR EXISTS (
                   SELECT 1 FROM pages far WHERE far.id = l.from_page_id AND far.deleted_at IS NULL AND far.source_id = ANY(${scope}))))
        )::int AS link_count
      FROM pages p
    ...
    return {
      ...
      most_connected: (connected as unknown as { slug: string; link_count: number }[]).map(row => ({
        slug: row.slug,
  ...
  async batchLoadEmotionalInputs(slugs?: string[], opts?: { sourceId?: string }): Promise<EmotionalWeightInputRow[]> {
    const sql = this.sql;
    const sourceId = opts?.sourceId;
    const pageFilter = slugs
      ? (sourceId
        ? sql`WHERE p.deleted_at IS NULL AND p.slug = ANY(${slugs}::text[]) AND p.source_id = ${sourceId}`
        : sql`WHERE p.deleted_at IS NULL AND p.slug = ANY(${slugs}::text[])`)
      : (sourceId
        ? sql`WHERE p.deleted_at IS NULL AND p.source_id = ${sourceId}`

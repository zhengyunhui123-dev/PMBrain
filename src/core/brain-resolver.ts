...
const DOTFILES = ['.pmbrain-mount'];
/** Same regex as brain-registry. Kept in sync. */
const BRAIN_ID_RE = /^[a-z0-9](?:[a-z0-9-]{0,30}[a-z0-9])?$/;

/**
 * Walk up from startDir looking for a .pmbrain-mount dotfile. Returns the
 ...
 * @param cwd  Working directory for .pmbrain-mount walk. Defaults to process.cwd().
 * @param mountsLoader  Override for testability. Returns the list of enabled
 *                      mounts. Defaults to reading ~/.pmbrain/mounts.json.
...
export function resolveBrainId(
  ...
  // 2. Env var.
  const envName = 'PMBRAIN_BRAIN_ID';
  const env = process.env.PMBRAIN_BRAIN_ID;
  if (env && env.length > 0) {

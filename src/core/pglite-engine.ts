...
import { validateSlug, contentHash, rowToPage, rowToStalePage, rowToChunk, rowToSearchResult, takeRowToTake, isUndefinedTableError, isUndefinedColumnError, warnOncePerProcess } from './utils.ts';
...
export function buildPgliteInitErrorMessage(
  ...
  switch (verdict) {
    ...
    case 'permission':
      hint =
        '  无法访问数据库路径（权限不足）。请检查目录权限、杀毒软件占用与当前用户是否可写。';
      break;
    case 'unknown':
    default:
      hint =
...
export class PGLiteEngine implements BrainEngine {
  ...
  private async applyForwardReferenceBootstrap(): Promise<void> {
    ...
    if (needsPagesLinksExtractedAt) {
      // Schema 115: pages_links_extracted_at_idx in PGLITE_SCHEMA_SQL
      // references links_extracted_at. Existing brains that already have
      // last_retrieved_at (and every older bootstrap target) still lack
      // this column; omitting it from the early-return above made Sidecar
      // skip ADD COLUMN and crash on schema replay.
      await this.db.exec(`
  ...
  async searchKeyword(query: string, opts?: SearchOpts): Promise<SearchResult[]> {
    ...
    const keywordSql = `WITH ranked AS (
         SELECT
  ...
  async getContentFlagsByPageIds(
    ...
    for (const row of rows as Array<{ id: number; reason: string | null; detail: string | null }>) {
      if (row.reason) {
  ...
  async listEnrichCandidates(opts: EnrichCandidatesOpts): Promise<EnrichCandidate[]> {
    ...
    const params: unknown[] = [opts.types, threshold];
    const where: string[] = [
      'p.deleted_at IS NULL',
      'p.type = ANY($1::text[])',
      `(char_length(p.compiled_truth) + char_length(COALESCE(p.timeline, ''))) < $2`,
    ];
    ...
    const { rows } = await this.db.query(
      `SELECT
       ...
       FROM pages p
       WHERE ${where.join(' AND ')}
       ORDER BY ${ENRICH_ORDER_SQL[orderKey]}
       LIMIT $${params.length}`,
      params,
    );
    return (rows as Record<string, unknown>[]).map((row) => ({
      ...
      type: row.type as EnrichCandidate['type'],

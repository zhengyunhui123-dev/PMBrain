...
PMBrain v0.33 将 `pmbrain skillpack` 从包管理器重塑为脚手架 + 参考库。本指南解释模型和工作流。

## 我们为什么改变它

v0.33 之前（"琥珀"模型）：

- `pmbrain skillpack install <name>` 将 bundled 技能复制到你的工作区，并在你的 `RESOLVER.md` / `AGENTS.md` 中写入一个 managed-block 围栏，并带有 `cumulative-slugs="..."` 收据。
- 后续安装进行哈希检查每个文件，并拒绝覆盖本地编辑，除非你传递 `--overwrite-local`。
- `pmbrain skillpack uninstall` 有自己的数据丢失保护措施（D8 收据门 + D11 内容哈希预扫描）并重建围栏。

它工作了，但它将个人 AI 技能视为供应商包。用户无法干净地 fork 技能，而后续的安装会与他们斗争。每次发布都会重新审理同一个托管块。仅托管块的测试表面就约 1000 行。

技能不是供应商包。它们是你的代理仓库中的一级代码。你脚手架一次，你拥有它们，你自由地 fork 和编辑。当 PMBrain 发布新版本时，你会问"什么改变了？" — 代理读取差异并决定要集成什么（如果有的话）。

## 五个命令

### `pmbrain skillpack scaffold <name> [--workspace PATH]`

一次性、附加地将 bundled 技能复制到你的仓库。拒绝覆盖任何存在的文件。路由来自每个技能的前置事务 `triggers:` 数组 — PMBrain 不触碰你的 `RESOLVER.md` 或 `AGENTS.md`（见下面的"代理如何发现脚手架技能"）。

```bash
cd ~/git/your-agent-repo
pmbrain skillpack scaffold book-mirror
...
### `pmbrain skillpack reference <name> [--workspace PATH] [--apply-clean-hunks] [--json]`

只读更新镜头。将 PMBrain 的包与你的本地副本进行比较，并发出每个文件状态（`identical` / `differs` / `missing`）以及任何 `differs` 条目的统一差异。

```bash
pmbrain skillpack reference book-mirror
# 这些文件作为参考位于 <pmbrain-path>。读取它们并
...
`reference <name> --apply-clean-hunks` 是自动应用路径。它解析 PMBrain 的包与你的本地副本之间的差异，应用每个其预更改上下文唯一匹配的 hunk。**双向合并限制**：没有脚手架时基跟踪，工具无法区分“PMBrain 更改了 X”和“你更改了 X”。应用 hunk 会将内容对齐到 PMBrain 参考包；先用 `--dry-run` 预览。

### `pmbrain skillpack migrate-fence [--workspace PATH] [--dry-run]`
...
```bash
cd ~/git/your-agent-repo
pmbrain skillpack migrate-fence
...
### `pmbrain skillpack scrub-legacy-fence-rows [--workspace PATH] [--dry-run]`
...
### `pmbrain skillpack harvest <slug> --from <host-repo-root> [--no-lint] [--dry-run]`

脚手架的逆操作：将经过验证的技能从主机仓库提升回 PMBrain 参考包，以便其他客户端使用。默认行为：

- 主机技能目录中的符号链接被拒绝（规范路径限制）。
- 隐私 linter 根据 `~/.pmbrain/harvest-private-patterns.txt` 以及内置默认值（规范私有 fork 名称、常见电子邮件正则表达式、Slack 频道模式）扫描收获的文件。任何匹配 → 回滚（删除收获的文件）并以非零退出。
...
这取代了 v0.32 模型，其中 `pmbrain skillpack install` 将表行写入你的 `RESOLVER.md`。行消失了（或者，对于从旧模型迁移的用户，由 `migrate-fence` 过渡保留，直到它们运行 `scrub-legacy-fence-rows`）。
...
v0.33 中没有 `pmbrain skillpack uninstall` 命令。你的 `skills/<slug>/` 中的文件是你仓库的一级成员 — 像任何其他代码一样删除它们：
...
- **新的主机仓库，想要 PMBrain 技能** → `scaffold`
- **PMBrain 发布了新版本，想要看看什么改变了** → `reference`（只读）或 `reference --apply-clean-hunks`（自动）
- **从 v0.32 或更早升级** → `migrate-fence`（一次性）
- **`migrate-fence` 后清理** → `scrub-legacy-fence-rows`
- **将你的 fork 的技能提升回 PMBrain** → `harvest` + `skillpack-harvest` 编辑技能

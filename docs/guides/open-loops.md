...
Admin「工作台 → 待我处理」只展示已自动发现的结果，可以标记已处理或不再跟踪。在「设置 → 自动维护」配置一次后，Gmail 同步、会议和 AI 对话由后台持续检查；页面上的「重新检查」只是异常补救。
...
对手方只用带 Source 的实体解析，不会把「张总」自动 slug 成另一个人。跨 Source 同名人物只会以「可能是同一个人」进入「待我处理」，用户确认后才关联。

...
下载地址：[GitHub Releases](https://github.com/zhengyunhui123-dev/PMBrain/releases)（当前最新：`PMBrain-Setup-1.0.22.exe`）

运行 `PMBrain-Setup-1.0.22.exe` 后，桌面端会优先读取现有 `.pmbrain/config.json`，并兼容读取旧版 `.gbrain/config.json`；已有数据库和 API Key 会直接沿用，只有本机没有配置时才打开首次配置向导。首次配置时，用户只需选择 PGLite 本地数据库或 Docker Postgres、填写所需模型 API Key，并选择知识库目录；桌面端会生成 `config.json`、初始化数据库、注册知识库 Source，并固定本机管理员 bootstrap token。
...
| 更新 | 说明 |
|------|------|
| **ChatGPT MCP Tunnel** | 通过 OpenAI Secure MCP Tunnel 安全接入 ChatGPT，无需开放公网端口；支持只读凭证隔离，写入和管理工具由服务端拒绝 |
| **Surrogate-pair 安全截断** | `safeSplitIndex` 修复 emoji / 4 字节 CJK 字符切半导致的 JSON 解析错误，已在 `synthesize.ts` 中应用 |
| **Windows 桌面端 1.0.22** | 修复安装包遗漏 PGLite 运行时、单实例无法唤醒窗口和失败状态误报；老用户正常启动直接进入管理台，故障时进入独立恢复页 |
